...
object Control {
  ...
  final def printContinuation(): Control[Unit] = 
    new Control[Unit] {
      def apply[R](k: MetaCont[Unit, R]): Result[R] = {
        println(k.toString)
        Computation(pure(()), k)
      }
    }

  final def use[A, Res](c: ContMarker[Res])(f: CPS[A, Res]): Control[A] =
    new Control[A] {
      def apply[R](k: MetaCont[A, R]): Result[R] = {
  ...
  final def delimitCont[Res](marker: ContMarker[Res])(f: marker.type => Control[Res]): Control[Res] =
    new Control[Res] {
      def apply[R2](k: MetaCont[Res, R2]): Result[R2] = {
  ...
  final def delimitState[Res](marker: StateMarker)(f: Control[Res]): Control[Res] =
    new Control[Res] {
      def apply[R2](k: MetaCont[Res, R2]): Result[R2] = {
  ...
  final def delimitCatch[Res](marker: CatchMarker[Res])(f: Control[Res]): Control[Res] =
    new Control[Res] {
      def apply[R2](k: MetaCont[Res, R2]): Result[R2] = {
